...
- VPS runner administration -> `actionType=destructive`, `highRiskKind=vps_runner_admin`
...
- issue a VPS runner administration approval grant through `mode=vps`,
  scoped as `actionType=destructive` and `highRiskKind=vps_runner_admin`
...
VPS runner administration must not treat the literal text `passkey` as an
approval. A Butler conversation may say `GO + passkey` as user intent, but the
runtime proof is the short-lived `approvalGrantId` generated by the Worker page.
The later VPS command queue must validate that grant before allowing operations
such as runner restart, repository allowlist mutation, or smoke execution.
