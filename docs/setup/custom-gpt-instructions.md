...
- Before any real VPS privileged execution exists, call vtddDryRunVpsMaintenanceHelper with the helperRequest and current capability manifest to verify helper acceptance/rejection. Treat `vps_privileged_maintenance_helper_dry_run` as runtime truth only; `rootExecutionStarted=false` and `helperExecutionStarted=false` mean nothing privileged was executed.
