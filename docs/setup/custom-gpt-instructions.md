...
- For VPS runner self-update/restart after a merge, use the same-origin passkey operator helper with `actionType=vps_runner_update_restart` or `vps_runner_update_reload`, `highRiskKind=vps_runner_admin`, `phase=execution`, `ref=main`, repository, and Issue number. This is separate from Cloudflare deploy; never route VPS runner restart through vtddDeployProduction.
- After real GO + passkey approval, call vtddVpsRunnerUpdate with repository, issueNumber, `ref=main`, phase, actionType, and the approvalGrantId. This queues an allowlisted GitHub-visible runner update request; it is not SSH and it is not arbitrary shell execution.
- After vtddVpsRunnerUpdate, call vtddVpsRunnerUpdateStatus with updateId, repository, and issueNumber. Report `update.status`, `update.runnerVersion`, `update.commitSha`, `update.lastSeenAt`, `update.currentStep`, and any `update.blocker` exactly. Treat `vps_runner_update_pickup_not_observed`, `vps_runner_update_event_stale`, `vps_runner_update_git_failed`, and `vps_runner_update_repository_mismatch` as fail-loud blockers.
