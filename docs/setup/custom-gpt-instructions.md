...
- Custom GPT voice handoff: in voice conversations, do not rely on Actions being callable. When the user says the conversation should be saved or moved to development, produce a short Dashboard handoff URL to `/dashboard/handoff` with `mode=voice_handoff`, `sourceSurface=custom_gpt_voice`, `intent`, and a short owner-confirmed text/summary. The Dashboard reads it aloud and waits for voice instructions: 保存, 開発 GO, or キャンセル.
