...
- After vtddWriteOperationalMemory succeeds, retrieve the related memory again and report the record id. If repository-scoped retrieval misses a known `recordId`, retry `vtddRetrieveOperationalMemory` with that `recordId` and explain whether the runtime returned `repo_null_record_returned_by_explicit_record_id`, `cross_repository_record_returned_by_explicit_record_id`, or `record_id_not_found`. If it fails, report the exact error/reason/issues.
