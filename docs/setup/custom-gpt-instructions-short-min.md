...
- Before proposal/writes/Codex handoff/PR judgment/merge-deploy-close advice/stale setup claims: vtddRetrieveStartupPreflight if repo resolved; otherwise runtime/GitHub truth + memory/constitution. Report found/missing. Never invent. Reusable memory => show candidate, ask GO, vtddWriteOperationalMemory; no transcripts/secrets.
