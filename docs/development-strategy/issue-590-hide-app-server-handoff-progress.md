...
Dashboard Butler PWA の通常 chat 本文には、`Codex app-server に渡しています` や `作業を継続しています。まだ最終回答は生成中です。` のような低情報 progress bubble が挟まらない。owner は composer 下の短い `進行中` 表示で待機状態を把握でき、最終返信と必要な owner action だけを chat 本文として読める。
...
Dashboard Butler owner-facing surface では、handoff / long-turn heartbeat は transient progress card に留める。`transient_progress_snapshot.progressSummary.entries` は chat 本文の checkpoint bubble として描画されるため、低情報 source/text を summary entry に入れない。最終返信への progress summary 添付や waiting approval / waiting user input の owner action は壊さない。

## 仮説

原因仮説は、`broadcastTransientStatus({ snapshot: true, snapshotSource: "owner_message_dispatch" })` と `long_turn_checkpoint` が `buildDashboardProgressSummarySnapshot()` で summary entry 化され、thread refresh / transient status restore 時に `renderThreadProgressCheckpoint()` が通常 BUTLER bubble として表示していること。
...
- `src/worker/runtime.js`: progress summary entry inclusion guard。risk は有用な progress checkpoint まで消すこと。
...
progress summary を絞りすぎると、owner action が必要な `waiting_approval` / `waiting_user_input` まで見えなくなる。今回の exclusion は owner handoff / long-turn heartbeat の低情報 source/text に限定する。
...
production PWA live E2E で長い app-server turn を走らせ、`Codex app-server に渡しています` と `作業を継続しています。まだ最終回答は生成中です。` が通常 chat bubble に出ないこと、composer 下の progress は残ること、最終返信は同じ thread に入ることを検証する。
