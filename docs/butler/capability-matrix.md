...
| Butler -> Codex Cloud pickup | `broken-live` | #157 live handoff execution `remote-codex-issue157-frn8c1` stayed `queued` with no branch/PR, and raw progress evidence recorded `Branch not found`: https://github.com/marushu/vtdd-v2-p/issues/157#issuecomment-4352013434. Queued/requested is correctly not treated as success. | Fix or replace the transport so a Butler-originated development handoff creates GitHub-visible branch/PR evidence, or returns a first-class blocker instead of indefinite queued state. |
