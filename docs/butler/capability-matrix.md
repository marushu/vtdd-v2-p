...
| issue create | `verified-live` | #163 source/runtime/surface updates were deployed, then live Butler exact Issue payload -> natural `GO` created #165: https://github.com/marushu/vtdd-v2-p/issues/165. Boundary checks blocked missing payload, changed title, and unresolved repo without asking for internal policy fields. | Continue readback/comment/update validation for follow-up Issue flows. |
...
| natural GO -> normal GitHub write | `verified-live` | #152 added `issue_create`; #163 source expands registry-backed binding to `issue_create`, `issue_comment_create`, and `pull_comment_create`. Live Butler happy-path evidence exists for all three configured operations: https://github.com/marushu/vtdd-v2-p/issues/165, https://github.com/marushu/vtdd-v2-p/issues/161#issuecomment-4351759028, and https://github.com/marushu/vtdd-v2-p/pull/163#issuecomment-4351775751. Boundary checks also blocked: no exact payload, changed title, and unresolved nickname/repo. | Keep this status limited to the configured natural-GO operations; validate update/branch/PR-create separately. |
...
| `@codex` handoff comment | `partial-live` | PR #155 produced a `deliveryMode=codex_cloud_github_comment` request comment with `@codex review`; #157 live handoff created a Butler-originated delegation comment: https://github.com/marushu/vtdd-v2-p/issues/157#issuecomment-4351974112. | Verify that a Butler-originated development handoff is picked up by Codex and produces branch/PR evidence. |
| Butler -> Codex Cloud pickup | `broken-live` | #157 live handoff execution `remote-codex-issue157-frn8c1` stayed `queued` with no branch/PR, and raw progress evidence recorded `Branch not found`: https://github.com/marushu/vtdd-v2-p/issues/157#issuecomment-4352013434. Queued/requested is correctly not treated as success. Source now must convert stale no-pickup state into `codex_cloud_pickup_not_observed` instead of indefinite queued state. | Re-run the Butler-originated development handoff and verify either GitHub-visible branch/PR evidence or the first-class blocker. |
...
| execution progress tracking | `partial-live` | #157 live tracking reported queued/requested, branch `null`, PR `null`, and raw failure `github_read_failed: Branch not found` without overclaiming success: https://github.com/marushu/vtdd-v2-p/issues/157#issuecomment-4352013434. Branch/PR success state remains unproven. | Track a repaired handoff through queued -> branch-created or PR-created, or return an explicit blocker. |
...
1. Issue read completeness: open/closed/exact body/comments/pagination
2. PR runtime truth completeness: PR state/diff/comments/reviews/checks/runs/branches
3. Butler -> Codex development handoff progress: requested/queued/blocked/branch/PR (#157)
