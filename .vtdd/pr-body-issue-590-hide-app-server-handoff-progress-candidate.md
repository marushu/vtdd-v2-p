...
- Issue #590 の部分進捗です。添付画像で確認された `Codex app-server に渡しています... usage_profile...`、`作業を継続しています。まだ最終回答は生成中です。`、`（続き生成中）` 付きの途中返信断片が通常 chat の `BUTLER` bubble に挟まる読みづらさを止めます。
- handoff / long-turn heartbeat / reply delta は composer 下の transient progress には残しますが、`progressSummary.entries` には入れず、通常 chat bubble として復元されないようにします。
...
- `Codex app-server に渡しています。...` の動的 handoff 文言は source 境界で progress summary entry から除外しました。
- `作業を継続しています。まだ最終回答は生成中です。` を低情報 progress として扱い、long-turn checkpoint を transient-only にしました。
- `app_server_reply_delta` は途中本文や `（続き生成中）` を出さず、`応答を生成しています。` の汎用 transient status だけに正規化しました。
- Unit / E2E evidence で、handoff status / continuation marker が transient-only で chat bubble を増やさないことを確認しました。
...
- 完了体験: Dashboard Butler PWA の通常 chat 本文には低情報 handoff / heartbeat / continuation bubble が挟まらず、owner は composer 下の `進行中` と最終返信だけを読める。
- VTDD 全体で進める部分: Issue #590 の owner-facing long-turn observability を改善する。PR #808 の auto-scroll rollback 後に残った履歴汚染だけを扱う。
- 設計: Dashboard Butler owner-facing surface では transient snapshot の `progressSummary.entries` が chat checkpoint bubble になるため、handoff / long-turn heartbeat / reply delta の source/text を summary entry から除外する。
- 仮説: 原因仮説は、`owner_message_dispatch`、`long_turn_checkpoint`、`app_server_reply_delta` が `progressSummary.entries` に入り、thread refresh / transient restore で `renderThreadProgressCheckpoint()` により通常 BUTLER bubble として描画されること。
...
- merge 後に通す E2E: production PWA live E2E で長い app-server turn を走らせ、`Codex app-server に渡しています`、`作業を継続しています。まだ最終回答は生成中です。`、`続き生成中` が通常 chat bubble に出ないこと、composer 下 progress と最終返信が残ることを検証する。
...
- Implementing Success Criteria: handoff / long-turn heartbeat / reply delta の低情報 progress を通常 chat bubble に混ぜない。
...
- What may break if we patch narrowly: 有用な progress checkpoint が減る可能性はあるが、今回の exclusion は handoff / long-turn heartbeat / reply delta に限定する。
...
- Queue delta: Issue #590 の handoff / continuation progress transient-only slice を進めます。PR #808 は merged 済みなので、新規 PR で同じ Issue #590 の Evidence Gap を減らします。
...
- file: `src/worker/runtime.js`
  - hypothesis: `buildDashboardProgressSummarySnapshot()` が handoff / long-turn heartbeat / reply delta を summary entry 化し、Dashboard renderer が通常 bubble として復元している。
...
- expected: handoff / long-turn heartbeat / reply delta を summary entry から外せば、赤丸の低情報 BUTLER bubble は通常 chat 履歴に出なくなる。
...
- lesson: transient snapshot の `progressSummary.entries` は通常 chat bubble として見えるため、低情報 status や途中返信断片を入れてはいけない。
...
- Owner goal: 長い作業中の handoff / heartbeat は見えるが、通常 chat 本文には低情報 progress bubble や途中返信断片が挟まらない。
