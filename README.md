...
- Cloudflare Worker runtime entry (`src/worker.js`) with `/health`, `/setup/wizard`, and `/v2/gateway` (legacy `/mvp/gateway` also accepted)
...
- machine auth path for `/v2/gateway` (Bearer token / Access service token)
