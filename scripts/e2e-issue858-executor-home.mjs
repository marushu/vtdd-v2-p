...
try{
 for(const colorScheme of ['light','dark'])for(const scenario of ['healthy','ready','hiccup','emergency','version','dirty','unsynced','pending','uninitialized']){
  ...
  if(scenario==='emergency'){clock=now+601000;s=applyNodeReport(s,{...mac,executorId:'vps',checkpoint:cp,observedAt:new Date(clock).toISOString(),heartbeatAt:new Date(clock).toISOString()},clock);}
  if(scenario==='dirty')s.checkpoint.dirty=true;
  if(scenario==='unsynced')s.nodes.vps.checkpoint.headSha='b'.repeat(40);
  ...
  if(scenario==='unsynced'){assert.match(text,/checkpointの同期不一致/);assert.equal(overview.executors.standbyReady,false);}
 ...
 for(const mode of ['planned','emergency','executor-enroll']) {
  ...
  await page.addInitScript(()=>{Object.defineProperty(navigator.credentials,'get',{value:async()=>({id:'test',rawId:new Uint8Array([1]).buffer,type:'public-key',response:{authenticatorData:new Uint8Array([1]).buffer,clientDataJSON:new Uint8Array([1]).buffer,signature:new Uint8Array([1]).buffer,userHandle:null}})});});
  await page.route('**/*',async route=>{const path=new URL(route.request().url()).pathname;if(path==='/operator')return route.fulfill({contentType:'text/html',body:html});if(path==='/v2/executors/overview')return route.fulfill({json:{nodePublicKeys:{}}});if(route.request().method()!=='POST')return route.abort();const body=route.request().postDataJSON();calls.push({path,body});return route.fulfill({json:path.endsWith('/challenge')?{sessionId:'synthetic',optionsJSON:{challenge:'YQ'}}:path.endsWith('/verify')?{approvalGrant:{approvalId:'synthetic'}}:{ok:true}});});
  ...
  assert.deepEqual(errors,[]);await page.screenshot({path:resolve(output,mode+'-operator.png'),fullPage:true});results.push({scenario:mode+'-operator',passed:true});await context.close();
 }
 await writeFile(resolve(output,'results.json'),JSON.stringify(results,null,2));
 console.log(`${results.length} synthetic browser cases passed: ${output}`);
