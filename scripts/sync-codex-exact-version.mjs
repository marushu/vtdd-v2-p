...
export function validateVersionExecutionRequest(p, now = Date.now()) {
  ...
  return { valid:true, executable:false, shell:false, blockedReason:'provider_bound_version_sync_contract_missing', requiresScopedApproval:true, packageSpec:{executorId:'vps',packageName:p.packageName,exactVersion:p.exactVersion}, rollbackSpec:p.previousVersion ? {executorId:'vps',packageName:p.packageName,exactVersion:p.previousVersion} : null };
}
