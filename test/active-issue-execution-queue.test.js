...
test("active issue execution queue names the next automatic implementation lane", () => {
  assert.equal(sectionBody("Now").includes("Issue #845: Mission Workstream Reconciliation"), true);
  assert.equal(sectionBody("Now").includes("updates durable Mission state"), true);
