...
test("deploy-production workflow enforces the MVP production deploy boundary", () => {
  ...
  assert.equal(workflow.includes("vtdd-dashboard-deploy-event"), false);
  assert.equal(workflow.includes("const changeSummary = inferCommitSummary(commitSubject, commitBody);"), true);
