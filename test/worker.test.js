...
test("worker setup wizard localizes capability readout labels to Japanese", async () => {
  ...
  assert.equal(html.includes("フローフェーズ"), true);
  assert.equal(html.includes("現在のフェーズ"), true);
  assert.equal(html.includes("次の機能"), true);
  assert.equal(html.includes("遷移トリガー"), true);
  assert.equal(html.includes("フロー進捗"), true);
  assert.equal(html.includes("完了フェーズ"), true);
  assert.equal(html.includes("現在のブロッカー"), true);
  assert.equal(html.includes("残りフェーズ"), true);
  assert.equal(html.includes("プロバイダー接続フェーズ"), true);
  assert.equal(html.includes("GitHub フェーズ"), true);
  assert.equal(html.includes("Cloudflare フェーズ"), true);
  assert.equal(html.includes("サービス接続モデル"), true);
  assert.equal(html.includes("GitHub 接続モデル"), true);
  assert.equal(html.includes("Cloudflare 接続モデル"), true);
  assert.equal(html.includes("接続タイプ"), true);
  assert.equal(html.includes("必要理由"), true);
  assert.equal(html.includes("サービス接続アクション可能性"), true);
  assert.equal(html.includes("GitHub アクション"), true);
  assert.equal(html.includes("Cloudflare アクション"), true);
  assert.equal(html.includes("今必要なユーザー操作"), true);
  assert.equal(html.includes("想定アクション種別"), true);
  assert.equal(html.includes("サービス接続摩擦"), true);
  assert.equal(html.includes("GitHub 摩擦"), true);
  ...
  assert.equal(html.includes("サービス復帰継続性"), true);
  assert.equal(html.includes("GitHub 復帰"), true);
  assert.equal(html.includes("Cloudflare 復帰"), true);
  assert.equal(html.includes("期待される復帰コンテキスト"), true);
  assert.equal(html.includes("人の再入場が必要か"), true);
  assert.equal(html.includes("責務分割"), true);
  assert.equal(html.includes("人が行うステップ"), true);
  assert.equal(html.includes("VTDD ステップ"), true);
  assert.equal(html.includes("プロバイダーステップ"), true);
  assert.equal(html.includes("認証境界分割"), true);
  assert.equal(html.includes("VTDD サービスアクセス"), true);
  assert.equal(html.includes("オペレーター bootstrap 権限"), true);
  assert.equal(html.includes("外部アカウント接続"), true);
  assert.equal(html.includes("ランタイムマシン認証"), true);
  assert.equal(html.includes("発行読取結果"), true);
  assert.equal(html.includes("発行可能状態"), true);
  assert.equal(html.includes("ブロッキングゲート"), true);
  assert.equal(html.includes("次の発行条件"), true);
  assert.equal(html.includes("権限形状"), true);
  ...
  assert.equal(html.includes("権限更新"), true);
  assert.equal(html.includes("更新トリガー"), true);
  assert.equal(html.includes("更新ゲート"), true);
  assert.equal(html.includes("更新スコープ"), true);
  assert.equal(html.includes("権限更新拒否"), true);
  assert.equal(html.includes("拒否理由"), true);
  assert.equal(html.includes("拒否境界"), true);
  assert.equal(html.includes("拒否後復帰"), true);
  assert.equal(html.includes("権限要求鮮度"), true);
  assert.equal(html.includes("鮮度要件"), true);
  assert.equal(html.includes("陳腐化要求拒否"), true);
  assert.equal(html.includes("鮮度復帰"), true);
  assert.equal(html.includes("権限要求リプレイ"), true);
  assert.equal(html.includes("リプレイリスク"), true);
  assert.equal(html.includes("リプレイ拒否"), true);
  assert.equal(html.includes("リプレイ復帰"), true);
  assert.equal(html.includes("権限要求バインディング"), true);
  assert.equal(html.includes("バインディング対象"), true);
  assert.equal(html.includes("バインディングドリフト"), true);
  assert.equal(html.includes("バインディング復帰"), true);
  assert.equal(html.includes("権限要求ターゲット"), true);
  assert.equal(html.includes("ターゲット文脈"), true);
  assert.equal(html.includes("ターゲットドリフト"), true);
  assert.equal(html.includes("ターゲット復帰"), true);
  assert.equal(html.includes("権限要求来歴"), true);
  assert.equal(html.includes("来歴ソース"), true);
  assert.equal(html.includes("来歴ドリフト"), true);
  assert.equal(html.includes("来歴復帰"), true);
});
...
test("worker setup wizard records approval-bound bootstrap request without granting authority", async () => {
  ...
  assert.equal(htmlJa.includes("封筒消費計画"), true);
  ...
  assert.equal(htmlJa.includes("封筒消費結果アウトカム"), true);
  assert.equal(htmlJa.includes("アウトカム状態"), true);
  assert.equal(htmlJa.includes("アウトカム失敗"), true);
  assert.equal(htmlJa.includes("アウトカム次の証跡"), true);
  assert.equal(htmlJa.includes("封筒消費監査"), true);
  assert.equal(htmlJa.includes("監査記録"), true);
  assert.equal(htmlJa.includes("監査失敗"), true);
  assert.equal(htmlJa.includes("監査保持"), true);
  assert.equal(htmlJa.includes("完了主張"), true);
  assert.equal(htmlJa.includes("現在の主張"), true);
  assert.equal(htmlJa.includes("まだ主張不可"), true);
  assert.equal(htmlJa.includes("主張成立条件"), true);
  assert.equal(htmlJa.includes("証跡読取結果"), true);
  assert.equal(htmlJa.includes("ランタイム証跡"), true);
  assert.equal(htmlJa.includes("ブロック中証跡"), true);
  assert.equal(htmlJa.includes("次の証跡"), true);
  assert.equal(htmlJa.includes("安全読取結果"), true);
...
test("worker setup wizard consumes verification-only envelope when no writes remain", async () => {
...
test("worker setup wizard selection capture fails closed without request token and rejects non-candidate ids", async () => {
  ...
  const baseReturnTo = "/setup/wizard?repo=sample-org/vtdd-v2&repo=other-org/another-repo&githubAppCheck=on";
  const { sessionCookie } = await unlockSetupWizard(env, baseReturnTo);
  ...
  const invalidCandidateResponse = await worker.fetch(
    new Request("https://example.com/setup/wizard/github-app/capture-installation", {
      method: "POST",
      headers: {
        "content-type": "application/json",
  ...
  assert.equal(invalidCandidateResponse.status, 422);
  const invalidCandidateBody = await invalidCandidateResponse.json();
  assert.equal(
...
test("worker setup wizard can request detected installation continuation from github block", async () => {
  ...
  assert.equal(
    jsonBody.githubAppSetupCheck.requestDetectedInstallationAction.pendingInstallationIdParam,
    "pending_installation_id"
  );
  assert.equal(
    jsonBody.githubAppSetupCheck.requestDetectedInstallationAction.pendingInstallationId,
    "125153871"
  );
  assert.equal(
    jsonBody.githubAppSetupCheck.guidance.includes(
...
test("worker setup wizard reports request-required blocked state after capture fail-closed redirect", async () => {
  ...
  assert.equal(response.status, 200);
  const body = await response.json();
  assert.equal(body.approvalBoundBootstrapSession.state, "request_required_for_capture");
  assert.equal(
  ...
  assert.equal(html.includes('name="pending_installation_id" value="125153871"'), true);
  assert.equal(html.includes("Record GO + passkey request"), true);
  assert.equal(html.includes("record_go_passkey_request_for_capture"), true);
  assert.equal(
    html.includes(
  ...
  assert.equal(html.includes("<strong>Action:</strong> <code>record_go_passkey_request</code>"), true);
...
test("worker setup wizard missing-only-installation guidance prefers same-flow detection before fallback write", async () => {
  ...
  assert.equal(body.githubAppSetupCheck.returnTo, "/setup/wizard?repo=sample-org%2Fvtdd-v2&githubAppCheck=on");
  assert.deepEqual(body.githubAppSetupCheck.guidance, [
