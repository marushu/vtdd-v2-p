...
test("worker serves v2 dashboard for allowed owner identity without exposing secrets", async () => {
  ...
  assert.equal(body.includes('id="butler-interrupt-panel"'), true);
  assert.equal(body.includes("実行中の割り込み指示"), true);
  assert.equal(body.includes("途中で伝えたいことを書く"), true);
  assert.equal(body.includes(".send-button.stop-state"), true);
  assert.equal(body.includes(".composer.is-running .composer-box"), true);
  assert.equal(body.includes("let executionRunning = false"), true);
  assert.equal(body.includes("function setExecutionRunning("), true);
  assert.equal(body.includes("function updateComposerControls()"), true);
  assert.equal(body.includes('submitButton.textContent = executionRunning ? "■" : "↑"'), true);
  assert.equal(body.includes('submitButton.setAttribute("aria-label", executionRunning ? "実行を停止" : "Butler に送信")'), true);
  assert.equal(body.includes("function sendInterruptMessage()"), true);
  assert.equal(body.includes('text: "割り込み指示: " + text'), true);
