...
test("worker serves v2 dashboard for allowed owner identity without exposing secrets", async () => {
  ...
  assert.equal(body.includes("チャット設定"), true);
  ...
  assert.equal(body.includes("--link:var(--butler-accent);"), true);
  assert.equal(body.includes("--owner-link:var(--butler-accent);"), true);
  assert.equal(body.includes("--code-bg:var(--butler-bg);"), true);
  assert.equal(body.includes("--code-text:var(--butler-ink);"), true);
  assert.equal(body.includes("--owner-code-bg:var(--butler-card);"), true);
  assert.equal(body.includes("--owner-code-text:var(--butler-ink);"), true);
  assert.equal(body.includes("--link:var(--butler-accent);"), true);
  assert.equal(body.includes("--owner-link:var(--butler-accent);"), true);
  assert.equal(body.includes("--code-bg:var(--butler-bg);"), true);
  assert.equal(body.includes("--code-text:var(--butler-ink);"), true);
  ...
  assert.equal(body.includes("background: var(--panel)"), true);
  assert.equal(body.includes("data-butler-header"), true);
  assert.equal(body.includes("data-butler-primary-nav"), true);
  ...
  assert.equal(body.includes("const threadProgressCheckpointCards = new Map()"), true);
  assert.equal(body.includes("const pacedProgressCheckpointTimers = new Map()"), true);
  ...
  assert.equal(body.includes("function scheduleGentleScrollFollow(shouldFollow)"), false);
  assert.equal(body.includes("scrollBy({ top:"), false);
  assert.equal(body.includes("scrollBy({ top: nextStep, behavior: \"smooth\" })"), false);
  assert.equal(body.includes("function scrollToLatestIfFollowing(shouldFollow)"), true);
  assert.equal(body.includes("scrollToLatestIfFollowing(shouldFollow, { gentle:"), false);
  ...
  assert.equal(body.includes('aria-label="チャット設定幅を変更"'), true);
  ...
  assert.equal(body.includes('actions.appendChild(meta)'), true);
  assert.equal(body.includes('entry.appendChild(actions)'), true);
  assert.equal(body.includes('target.appendChild(entry)'), true);
  ...
  assert.equal(body.includes(".copy-message { position: absolute; top: -8px; right: -8px;"), false);
  assert.equal(body.includes(".bubble.has-copy-action:hover .copy-message"), false);
  assert.equal(body.includes(".bubble.actions-visible .copy-message"), false);
  assert.equal(body.includes(".copy-message { width: 28px; height: 28px; flex: 0 0 auto; opacity: .88; }"), true);
  ...
  assert.equal(body.includes("システムメッセージをコピー"), true);
  ...
  assert.equal(body.includes("function attachMessageActionReveal("), false);
  assert.equal(body.includes('article.classList.add("has-copy-action")'), false);
  assert.equal(body.includes('article.classList.toggle("actions-visible")'), false);
  assert.equal(body.includes('event.target.closest("a, button, input, textarea, select, summary")'), false);
  assert.equal(body.includes("article.tabIndex = 0"), false);
  assert.equal(body.includes('article.addEventListener("keydown"'), false);
  assert.equal(body.includes('message.role === "owner" ? "owner" : message.role === "system" ? "system" : "butler"'), true);
  assert.equal(body.includes('if (message.role === "butler") {'), true);
  ...
  assert.equal(body.includes("--butler-viewport-height"), true);
  assert.equal(body.includes("overflow: hidden"), true);
  ...
  assert.equal(aliasBody.includes("チャット設定"), true);
...
test("worker serves human-facing dashboard pages for every management menu", async () => {
  ...
  for (const [route, title, rawLabel] of routes) {
    ...
    assert.equal(body.includes("{\"ok\""), false);
    assert.equal(body.includes('data-butler-primary-nav'), true);
    assert.equal(body.includes('data-butler-menu'), true);
    assert.equal(body.includes('aria-controls="butler-shell-menu-links"'), true);
    assert.equal(body.includes('data-butler-header'), true);
    assert.equal(body.includes("butler-shell-menu-links"), true);
    assert.equal(body.includes("--butler-nav-height"), true);
...
test("worker serves passkey operator page", async () => {
  ...
  const html = await response.text();
  assert.equal(html.includes("変更の取り込みを確認"), true);
