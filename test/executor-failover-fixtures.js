...
export const report = (id='mac',n=now) => ({executorId:id,generation:1,observedAt:stamp(n),heartbeatAt:stamp(n),codexVersion:'1.2.3',appServerSmokeOk:true,serviceState:'inactive',checkpoint:checkpoint(n)});
export const seed = (n=now) => ({primaryExecutor:'mac',standbyExecutor:'vps',approvedCodexVersion:'1.2.3',macReport:report('mac',n)});
export const transition = () => ({expectedGeneration:1,executorFrom:'mac',executorTo:'vps',issueNumber:858,targetConfirmed:true,reason:'owner_manual_transition',transitionMode:'planned',primaryIsolationConfirmed:false});
