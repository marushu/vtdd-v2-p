...
test("Butler capability matrix records live/source/unverified status without overclaiming", () => {
  ...
  assert.equal(doc.includes("| natural GO -> normal GitHub write | `verified-live` |"), true);
  assert.equal(doc.includes("registry-backed binding to `issue_create`, `issue_comment_create`, and `pull_comment_create`"), true);
  assert.equal(doc.includes("Live Butler happy-path evidence exists for all three configured operations"), true);
  assert.equal(doc.includes("Boundary checks also blocked: no exact payload, changed title, and unresolved nickname/repo"), true);
  ...
  assert.equal(doc.includes("Boundary checks blocked missing payload, changed title, and unresolved repo"), true);
...
test("Butler capability matrix prioritizes the Butler-to-Codex path and surface guidance", () => {
  ...
  assert.equal(doc.includes("`natural GO -> normal GitHub write` boundary proof (#151/#161)"), false);
  assert.equal(doc.includes("Issue read completeness: open/closed/exact body/comments/pagination"), true);
